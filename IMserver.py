...
class NickBot(Bot):
    ...
    def ProcessMessage(message, client):
        to  = message.to
        frm = message.frm
        if NickBot.isUsernameAvailable(to, frm):
            NickBot.RegisterClient(to, frm, client)
#            NickBot.ConfirmToClient(client)
            print frm + " registered"
    
    @staticmethod    
    def RegisterClient(to, frm, client):
        if to not in MessageServer.directory.keys():
            fd = {}
            fd[frm] = client
            MessageServer.directory[to] = fd
        else:
            MessageServer.directory[to][frm] = client
    
    @staticmethod
    def isUsernameAvailable(to, frm):
        if to not in MessageServer.directory.keys():
          return True
        group = MessageServer.directory[to]
        if frm in group:
            return False
        else:
            return True
...
class MessageDecoder:
    ...
    def decode(self, raw_message):
        ...
        message = Message(str(dic["To"]), str(dic["From"]), str(dic["Message"]))
        return message
...
class MessageServer(tornado.websocket.WebSocketHandler):
    ...
    def on_message(client, raw_message):
        ...
        if message.to not in MessageServer.directory.keys():
            NickBot.ProcessMessage(message, client)
        elif client not in MessageServer.directory[message.to].values():
            if message.to != "NickBot":  # and isn't trying to do so
                MessageServer.close(client)      # we close his connection
            else:                                # If he is, NickBot handles it
                NickBot.ProcessMessage(message, client)
        groupDict = MessageServer.directory[message.to]
        for user, fd in groupDict.iteritems():
            ret = user +": " + message.msg
            fd.write_message(ret)

        """
        else:
            if message.target != "NickBot":
                MessageServer.close(client)
            else:
                print "again"
                NickBot.ConfirmToClient(client)
            return
...
#    application = tornado.web.Application([(r"/ws", MessageServer), ])
    application = tornado.web.Application([(r"/", MessageServer), ])
